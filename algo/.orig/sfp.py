"""
sfp.py - Streaming Flow Policy (SFP) on Push-T and robomimic.
...
Usage
-----
    python sfp.py --task pusht  --mode train --epochs 1000
    python sfp.py --task can    --mode train --epochs 1000
    python sfp.py --task pusht  --mode eval  --ckpt 1000 --seeds 50 --perturbs 0.0-2.0
    python sfp.py --task square --mode video --ckpt 1000 --seed 0
...
# 4. Model
# =============================================================================
def build_nets(g, arch="unet", lip=5.0, contr_backbone="unet"):
...
def load_ema_nets(g, task, ckpt, device, ckpt_path=None, tag="", arch=None, lip=None):
    ...
    if ckpt_path:
        ...
        # before the outputs/<task>/sfp/ split are still loadable.
        out_dir = ROOT / "outputs" / task
        candidates = [out_dir / f"sfp{tag}" / f"ep{ckpt}.ckpt",
                      out_dir / f"sfp_ep{ckpt}.ckpt",
    ...
    nets = build_nets(g, arch=arch, lip=lip, contr_backbone=cb).to(device)
...
def train(g, task, epochs, smoke, device=None, train_seed=0, tag="", arch="unet", lip=5.0, contr_backbone="unet"):
    ...
    ckpt_dir = ROOT / "outputs" / task / f"sfp{tag}"
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    nets = build_nets(g, arch=arch, lip=lip, contr_backbone=contr_backbone).to(device)
    ...
    for epoch in tqdm(range(epochs), desc=f"SFP-{task}"):
        losses = []
        for nbatch in dataloader:
            ...
            a, v = SampleCFMInputsAndTargets(xit, dxidt, t, k, sigma0)
            a, v = a.unsqueeze(1), v.unsqueeze(1)

            current_flat = nobs.flatten(start_dim=1)
...
def rollout(g, ema_nets, env, seed=0, perturb_level=0.0, max_steps=250,
            action_horizon=8, save_vis=False, device=None, obs_noise=0.0, act_noise=0.0):
    """Run one closed-loop episode. Returns (score, frames, steps).

    obs_noise: std of Gaussian noise added to the chunk-start conditioning window
    ...
    while not done:
        ...
        if obs_noise > 0:
            ncur = ncur + obs_noise * torch.randn(ncur.shape, generator=gen, device=device).cpu().numpy()
        with torch.no_grad():
            for i in range(action_horizon):
                ...
                if done:
                    break
                # open loop: the chunk-start window is held for the whole chunk
...
def sweep_path(task, seeds, out_json=None, action_horizon=None, tag="", obs_noise=0.0, act_noise=0.0):
    ...
    return (ROOT / "outputs" / task / f"sfp{tag}" / "eval"
            / f"sweep_s{seeds}{ahtag}{ontag}.json")
...
def _load_sweep(path, task, seeds, g, tag=""):
    ...
    return {
        ...
        "rollout": "sfp",
        "trained_by": "sfp",
        "method": "sfp",   # kept for older readers; equals trained_by
        "tag": tag,
        "task": task,
        "seeds": seeds,
...
def eval_one(g, task, seeds, ckpt, perturbs, smoke, device=None,
             action_horizon=8, ckpt_path=None, tag="", on_result=None, obs_noise=0.0, act_noise=0.0):
    ...
    for perturb in perturbs:
        ...
        for s in tqdm(range(seeds), desc=f"ep{ckpt} perturb{perturb}",
            ...
            score, _, nsteps = rollout(g, ema_nets, env, seed=s,
                               ...
                               action_horizon=action_horizon, obs_noise=obs_noise, act_noise=act_noise)
...
def evaluate(g, task, seeds, ckpts, perturbs, smoke, device=None,
             ...
             ckpt_path=None, tag="", obs_noise=0.0, act_noise=0.0):
    ...
    path = sweep_path(task, seeds, out_json, action_horizon, tag, obs_noise, act_noise)
    ...
    if not out_json and not ckpt_path and not obs_noise and not act_noise:   # noise runs are a different policy
    ...
    for ckpt in ckpts:
        ...
        probe = Path(str(ckpt_path)) if ckpt_path else (ROOT / "outputs" / task / f"sfp{tag}" / f"ep{ckpt}.ckpt")
        ...
        results = eval_one(g, task, seeds, ckpt, todo, smoke, device=device,
                           ...
                           on_result=_save, obs_noise=obs_noise, act_noise=act_noise)
...
def main():
    ...
    ap.add_argument("--arch", default="unet", choices=["unet", "contractive"],
                    help="contractive: v = mdot - k(a-m) + lip*psi, psi lip-Lipschitz in a "
                         "(dv/da <= -(k-lip) I everywhere by construction)")
    ...
    ap.add_argument("--tag", default="",
                    help="suffix on the checkpoint dir, e.g. _seed1 -> outputs/<task>/sfp_seed1/")
    ...
    ap.add_argument("--out-json", default=None,
                    help="where to write eval results "
                         "(default outputs/<task>/sfp/eval/sweep_s<seeds>.json)")
    args = ap.parse_args()
    ...
    if args.mode == "train":
        train(g, args.task, args.epochs, args.smoke, device=device,
              train_seed=args.train_seed, tag=args.tag, arch=args.arch, lip=args.lip, contr_backbone=args.contr_backbone)
    elif args.mode == "video":
        ckpt = parse_ckpt(args.ckpt)
        if isinstance(ckpt, list):
            ckpt = ckpt[-1]
        out = args.out or (f"{args.task}_sfp_ep{ckpt}"
        ...
        evaluate(g, args.task, args.seeds, ckpts, parse_perturb(args.perturbs),
                 ...
                 ckpt_path=args.ckpt_path, tag=args.tag, obs_noise=args.obs_noise, act_noise=args.act_noise)
