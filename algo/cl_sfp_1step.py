"""
cl_sfp_1step.py - CL-SFP weights with replanning every step (action horizon 1).

Same trained weights as CL-SFP: checkpoints are read from outputs/<task>/cl_sfp/.
Completes the 2x2 ablation {SFP, CL-SFP weights} x {chunk of 8, replan every
step}: the flow clock is reset to t=0 at every environment step, so the
field is only ever queried in its high-noise regime, with time-aligned
weights. Results land under outputs/<task>/cl_sfp_1step/eval/.

Usage
-----
    python cl_sfp_1step.py --task pusht --mode eval --ckpt 1000 --seeds 100 --perturbs 0.0-2.0
    python cl_sfp_1step.py --task can   --mode eval --ckpt 100-600 --seeds 100 --perturbs 0.0
    python cl_sfp_1step.py --task pusht --mode video --ckpt 1000 --seed 0 --perturb 1.4
"""
