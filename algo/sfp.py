...
-----
    ...
    python sfp.py --task pusht  --mode eval  --ckpt 1000 --seeds 100 --perturbs 0.0-2.0
...
a chunk-start observation goes stale against.
"""
...
def load_ema_nets(g, task, ckpt, device, ckpt_path=None, tag=""):
    ...
    an SFP model executed under the CL rollout.
    """
    if ckpt_path:
        path = str(ckpt_path)
    else:
...
def rollout(g, ema_nets, env, seed=0, perturb_level=0.0, max_steps=None,
    ...
    SFP execution: the conditioning window is read once per chunk and held for
    all `action_horizon` steps, i.e. open loop within the chunk. This is
    Algorithm 2 of the paper; actions stream out one per network call, but the
    observation the field sees is the chunk-start snapshot."""
    device = device or torch.device("cuda")
    obs_horizon = g["obs_horizon"]
    pred_horizon = g["pred_horizon"]
    action_dim = g["action"].shape[-1]
