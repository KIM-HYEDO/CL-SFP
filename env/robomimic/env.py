...
class RobomimicLowdimWrapper(gym.Env):
    ...
    def __init__(self,
        ...
        # With abs_action the policy speaks [pos3, rot6d, grip] per arm (10
        # dims) and step() converts to the [pos3, axis-angle, grip] robosuite's
        # OSC controller expects in control_delta=False mode.
        ...
        act_dim = self.n_arms * 10 if abs_action else env.action_dimension
        low = np.full(act_dim, fill_value=-1.0)
        high = np.full(act_dim, fill_value=1.0)
    ...
    def current_eef_action(self):
        """Absolute action that holds the current pose: [pos, rot6d, open] per arm.

        The flow starts here at t=0 in abs mode - the analogue of the zero
        delta ("stay put") the delta-action rollout starts from.
        """
        from env.robomimic.rotation import matrix_to_rot6d, quat_xyzw_to_matrix
        raw = self.env.get_observation()
        parts = []
        for k in range(self.n_arms):
            pos = raw[f"robot{k}_eef_pos"]
            r6 = matrix_to_rot6d(quat_xyzw_to_matrix(raw[f"robot{k}_eef_quat"]))
            parts.append(np.concatenate([pos, r6, [-1.0]]))     # -1 = gripper open
    ...
    def step(self, action):
        ...
        if self.abs_action:
            from env.robomimic.rotation import abs10_to_abs7
            action = abs10_to_abs7(action)
...
def make_env(dataset_path: str,
             ...
             abs_action: bool = False) -> RobomimicLowdimWrapper:
    ...
    if abs_action:
        # same controller, absolute goals: the dataset's actions_abs were
        # generated by this very controller, so nothing else may change
    ...
    return RobomimicLowdimWrapper(env=robomimic_env, obs_keys=obs_keys,
                                 ...
                                 perturb_object=perturb_object, abs_action=abs_action)
