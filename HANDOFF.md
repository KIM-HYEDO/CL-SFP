...
### DP 외란 커브 — can·square (2026-09-25 완료)

`run_dp_drift.sh`, 그림 `outputs/perf_dp_compare.png`. DP 3시드, SFP와 같은 9레벨·100 seeds.

| can (mm/step) | 0 | 0.5 | 0.75 | 1.0 | 1.25 | 1.5 | band 0.5–1.25 |
|---|---|---|---|---|---|---|---|
| SFP | .937 | .817 | .673 | .353 | .120 | .043 | .491 |
| DP | .953 | .853 | .630 | .393 | .183 | .097 | .515 |
| **CL-SFP+i** | .947 | .877 | **.747** | **.543** | **.330** | **.163** | **.624** |
| +i − DP, p | — | .42 | **<.001** | **<.001** | **<.001** | .015 | **+.109, p<1e-4** |

| square (mm/step) | 0 | 0.3 | 0.5 | 0.75 | 1.0 | 1.25 | 1.5 | band 0.3–1.25 |
|---|---|---|---|---|---|---|---|---|
| SFP | .750 | .633 | .417 | .250 | .117 | .080 | .047 | .299 |
| DP | .800 | .730 | .603 | .407 | .197 | .103 | .033 | .408 |
| **CL-SFP+i** | .800 | .670 | .543 | .347 | .210 | .140 | **.087** | .382 |
| +i − DP, p | — | .09 | .10 | .07 | .71 | .15 | **.004** | −.026, p=.06 |

**읽는 법.**
- **can: 폐루프 스트리밍이 DP를 이긴다.** 정적은 동률(.947 vs .953)인데 0.75 mm부터 CL-SFP+i가
  DP를 +.12~.15 앞서고(레벨별 p<.001, 밴드 pooled +.109, p<1e-4) 극단에서 DP는 SFP 수준으로
  내려앉는다. DP의 8스텝 open-loop chunk가 관측 낡음에 취약하다는 이론적 예상이 그대로 나온
  **논문의 핵심 그림**.
- **square: 중간 밴드에선 DP가 약간 위, 극단에서 역전.** 0.3–0.75 mm에서 DP가 +.04~.06
  (개별 p .07–.26, 유의하지 않음), 1.0 mm에서 교차, 1.5 mm에서 CL-SFP+i가 +.053 (p=.004).
  밴드 pooled −.026 (p=.06). "이긴다"고 쓰면 안 되고 "극단 외란에서만 역전"이 정확하다.
- DP는 두 태스크 모두 SFP보다 강인하다(밴드 +.02/+.11) — 결합 denoising 자체가 어느 정도
  강인성을 주며, 폐루프의 이득은 그 위에서 측정돼야 한다.
- 절대 수치 주의: DP square 정적이 정적 스윕 argmax에서는 .827, 외란 파일의 0.0에서는 .800.
  같은 seeds인데 .027 차이 = 스윕 argmax의 winner's curse + cuDNN 비결정성. 모든 방법에
  동일하게 걸리지만 표에 두 숫자가 다르게 나올 수 있음을 알고 있을 것.

### Diffusion Policy를 같은 프로토콜로 (2026-09-25 완료)

`algo/dp.py` — DP-C(1D UNet + DDPM 100)를 SFP와 **완전히 같은 조건**(델타 액션, 창 2/16/8,
UNet 폭, AdamW/cosine/EMA/batch 1024/1000 epoch, 250/700 예산, 100 eval seeds, argmax ckpt)으로.
차이는 방법뿐: 16스텝 chunk 결합 denoising + chunk당 DDPM 100 스텝 후 8스텝 open-loop.

| 정적 | SFP | CL-SFP+i | SFP(절대) | **DP (우리)** | DP 논문 |
|---|---|---|---|---|---|
| square | .750 | .800 | .750 | **.827 ±.02** | .96 avg / 1.00 max |
| tool_hang | .240 | .337 | .433 | **.887 ±.03** | .93 avg / 1.00 max |

**결론.**
1. **tool_hang의 격차는 방법이다.** 같은 데이터·모델·예산에서 DP .89 vs SFP 계열 .24–.43.
   DP는 ep100에 이미 .82이고 과적합 하락도 거의 없다. 스트리밍(액션 하나씩 Euler)과
   결합 denoising(16스텝을 한 번에)의 차이가 정밀·장기 태스크에서 50포인트 이상으로 벌어진다.
   앞 절의 "레시피 격차" 해석은 square에만 맞고 tool_hang에는 틀렸다 — 여기서 정정.
2. **square의 격차는 작다.** DP .83 vs CL-SFP+i .80 vs SFP .75. DP 논문의 .96과 우리 DP의 .83
   차이(.13)는 레시피(논문은 3000 epoch급 학습, 절대 액션 등)로 보인다.
3. 이 프로젝트의 폐루프 주장은 **SFP 대비 상대 비교**로는 유효하지만, 절대 성능에서 SFP 계열이
   DP에 크게 뒤지는 태스크(tool_hang)가 있다는 사실을 논문에 숨길 수 없다. 프레이밍은
   "DP보다 낫다"가 아니라 "10배 낮은 지연의 스트리밍 정책에서 폐루프가 강인성을 산다"여야 하고,
   DP 행을 표에 그대로 넣어야 한다.
4. 비용: DP 평가는 에피소드당 net 호출이 SFP의 12배(square 17.5초 vs 3.7초). 외란 커브까지
   DP로 뽑으려면 square ~5h, tool_hang ~14h 규모. DP의 외란 강인성 커브는 **아직 없다** —
   폐루프 SFP가 DP보다 외란에 강한지는 열린 질문.
...
3. 왜 폐루프가 무너지는지는 **미확정**. "현재 자세를 복사해 제자리에 멈춘다"는 가설은 기각됐다
   ...
   1~8스텝 낡아 그 지름길이 약하다. **적합도 검사로 뒷받침됨** (tool_hang seed0, 8스텝 flow
   적합도 비율 ↓=데모 재현 잘함): SFP 델타 .460 / 절대 .276, CL-SFP+i 델타 .413 / **절대 .152**.
   CL-SFP+i 절대 모드가 네 모델 중 데모를 압도적으로 잘 재현하는데 성공률은 SFP 절대(.45)의
   절반(.26) — 학습셋에서만 통하는 지름길을 배웠다는 전형적 신호.
